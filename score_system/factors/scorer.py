...
class EnhancedStrengthScorer:
    ...
    # 计算 log return 并结合 1h/4h/24h 涨幅衡量短期强度，归一化为 [0, 1]
    ...
    # 判断 EMA 多头排列（ema5 > ema10 > ema20）代表趋势强劲，否则弱
    ...
    # 利用 VolumeHeatmap 对近期交易量进行分类评分，衡量资金活跃度
    ...
    # RSI 反转预警机制：超买弱（<0.5），超卖强（>0.5），中间地带为中性
    ...
    # 计算相对动量（alpha）：剥离beta后的超额收益，衡量是否跑赢BTC
